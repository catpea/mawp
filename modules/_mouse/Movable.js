...
export default class Movable {
  ...
  start() {
    ...
    if(!position==='absolute') this.element.style.position = 'absolute'; // Ensure the element is absolutely positioned

    this.element.querySelectorAll('.card-header').forEach(el=>el.addEventListener('mousedown', this.mouseDownHandler));
    document.addEventListener('mouseup', this.mouseUpHandler);
    return () => this.stop();
  }

  stop() {
    this.element.querySelectorAll('.card-header').forEach(el=>el.removeEventListener('mousedown', this.mouseDownHandler));
  ...
  mouseMoveHandler(event) {
    ...
    if (this.#dragging) {
      ...
      this.system.source.settings.set('left', 'value', x);
      this.system.source.settings.set('top', 'value', y);
