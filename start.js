...
class EchoWorker extends Worker {
  processor(input, state) {
    console.log('EchoWorker outputting data!', input, state);
  }
}
...
class RouteListener extends Worker {
  processor({ url }, { method='GET'}) {
    ...
    const res1 = { statusCode: 200, end: (v) => { this.send('output', v) }, };
    server.handleRequest(req1, res1);
  }
}
...
const routeListener = scene.createActor('a', RouteListener, {});
scene.createActor('b', EchoWorker, {});
scene.createActor('x', QuickPipe, {from:'a:output',to:'b:input'});
